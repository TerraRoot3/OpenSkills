...
7. The left panel groups local branches, worktrees, remote branches, and tags in that order. Groups can collapse independently, and the divider can be dragged or adjusted with the keyboard to resize the persisted branch-list width. The commit graph loads up to 200 commits across all refs. Single-clicking a local or remote branch only locates and selects its tip commit in that loaded graph; it does not call `get_git_snapshot`, change the graph scope, select the branch row, or change the worktree. Double-clicking starts the confirmed switch flow, matching OpenGit. Tags and branch-backed worktrees remain read-only history navigation.
...
12. While a repository view is visible, the workbench checks a lightweight local Git signature every three seconds and on window focus. It reloads the full all-ref snapshot only after HEAD, refs, tags, worktrees, index, fetch/merge/rebase state, or porcelain status changes. External branch switches follow the new current branch.
...
- Single-clicking a local or remote branch only selects and centers the already loaded tip commit. Do not call `get_git_snapshot`, change `selectedRef`, or add a selected state to the clicked branch row. Double-clicking the branch or using its arrow control starts the same confirmed switch flow. Delay the single-click locator long enough to distinguish a double-click so the first click cannot run before checkout confirmation.
...
- The commit graph uses all refs, while `currentBranch` remains the authority for Pull, Push, checkout state, and the current-branch marker.
- A successful branch switch is followed by a fresh authoritative snapshot. The actual Git HEAD drives the top current-branch label and the current markers in the local and remote branch lists; do not preserve a stale history selection across an external branch switch.
