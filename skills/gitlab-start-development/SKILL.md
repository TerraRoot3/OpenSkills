...
description: Use when starting a new task in a GitLab repository where the user should prepare the local worktree first and the real GitLab issue and branch naming must match the hosting project exactly.
---
...
Use this skill to start GitLab development work after the user has already prepared the intended local worktree: validate that worktree context, sync the latest mainline branch, create the real GitLab issue, derive the branch name from the returned issue number, create the remote branch from the latest mainline, fetch it locally, and switch the prepared worktree onto that branch.
...
- the user already has, or can create first, the clean worktree that should hold the new task branch
...
- the user expects this skill to create the worktree for them instead of preparing it first
...
2. Record the current repo state.
   ...
   - If there are unrelated dirty changes, do not stash them away by force. Stop and ask the user to create or switch to a clean worktree first.
3. Confirm the intended worktree already exists.
   - Treat the current worktree as user-prepared input, not something this skill should create.
   - Capture the current worktree path.
   - If repo docs specify a worktree convention or location, verify that the current worktree follows it.
   - If the user has not prepared the target worktree yet, stop and ask them to do that before continuing.
4. Resolve the mainline branch.
   - Use repo docs if they specify it.
   - Otherwise use the GitLab project default branch or `origin/HEAD`.
5. Sync the local mainline branch to the latest remote mainline.
   - Expect the prepared worktree to start from the intended mainline branch or another repo-approved starting point.
   - Checkout the mainline branch locally in that prepared worktree if needed.
   - Pull the latest `origin/<mainline>` before creating the issue branch.
   - Do not create a task branch from stale local history.
6. Create the real GitLab issue.
...
8. Create the remote branch from the latest mainline.
...
9. Fetch the remote branch locally.
   - Fetch `origin/<branch>` into a matching local branch name.
   - Set local tracking to `origin/<branch>`.
10. Switch the prepared worktree onto the correct local branch.
    - Checkout the fetched local branch in the current worktree.
    - Confirm the current worktree is now on the correct tracking branch.
11. Only after the correct branch is checked out in the prepared worktree, create repo-local issue/design/ipml docs if the repo requires them.
    - Put those docs in the correct worktree and correct branch from the start.
12. If a mistaken local branch was created before the real issue existed:
    - migrate any useful local docs or notes into the correct branch if needed
...
- The user creates the worktree before this skill runs.
- Do not create a new worktree as part of this skill.
- One task branch should map to one clean, user-prepared worktree.
- Do not reuse a dirty working directory for a new issue branch.
- If the repo already has a preferred worktree convention, require the prepared worktree to follow it.
...
- If the correct worktree does not already exist, stop and tell the user to create it first instead of creating it on their behalf.
...
- If the remote branch exists but local fetch or branch checkout in the prepared worktree fails, keep the remote issue/branch intact and report the partial state clearly.
...
- local worktree path used
- current checked-out branch in that worktree
- any mistaken local branch cleaned up
