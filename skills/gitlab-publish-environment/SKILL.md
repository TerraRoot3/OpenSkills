...
**Default rule: publish by merging the full source branch into the target branch.** Do not silently replace the merge flow with cherry-pick, selective release branches, or isolated one-off publish branches just because the source branch was cut from a different base branch. If the user wants an isolated publish that excludes other source-branch history, or the repository has an explicit release-isolation rule, stop and confirm that exception before changing the flow.
...
- A source branch that was created from `main` or `master` can still be published to `dev` or `release` through the normal merge workflow if that is what the user asked for.
- Do not “protect” the target branch by inventing an isolated cherry-pick publish flow unless the user explicitly requests that isolation or repo documentation requires it.
- If you believe the source branch contains extra history the user may not intend to publish, surface that as a brief confirmation before deviating from the standard merge workflow.
...
21. Report the source commit, any source-branch sync merge result when `master` was merged into the source branch for production publish, the target merge commit, push results, target branch, tag result when applicable, and the final checked-out branch.
...
- Do not silently substitute cherry-pick, selective publish branches, or isolated release flows for the standard merge workflow. Use those only when the user explicitly asks for an isolated publish or when repository rules require it.
